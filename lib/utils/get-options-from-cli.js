...
const VALID_COMMANDS = commands.NAMES; // массив допустимых команд

/**
 * Разбор параметров и опции (начинаются с - или --) командной строки.
 *
 * @param argv
 * @return { command: '', builds: [], fields: {}, transport: {}, config: {}, transport: {}, fields: {} }
 */
function getOptionsFromCli(argv) {
  // Вернет объект { _: [] }, если будут параметры, они попадут в свойство "_" текущего объекта,
  // если будут опции они будут как свойства текущего объекта.
...
/**
 * Разбор параметров и опции командной строки и дальнейшая подготовка объекта с опциями
 *
 * @param cli
 * @returns {{command: *|string, builds: Array, fields: {}, transport: {}}}
 */
function transformCliToOptions(cli) {
  const options = {
    command: cli._[0], // предполагается что первый параметр (при его наличий) - команда из константы VALID_COMMANDS
    builds: [],
    fields: {},
    transport: {}
  };

  let args = cli._.slice(1); // возвращает параметры исключая первый, т.к. он присвоен в 43 строке
  if (VALID_COMMANDS.indexOf(options.command) === -1) {
    options.command = 'publish';
    args = cli._; // возвращает все параметры
  }

  // если 2й параметр .json файл
...
/**
 * Вывод справочной информации с возможными параметрами и опциями
 *
 * @param message
 */
