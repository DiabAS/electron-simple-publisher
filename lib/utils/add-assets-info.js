...
/**
 * Добавляет к build локальные пути к файлам обновлений (ASSETS_DATA)
 * и общую папку, где лежат все файлы обновлений
 *
 * @param build
...
/**
 * Возращает существующие пути, до файлов обновлений, заменяя шаблоны на действительные данные,
 * у не существующих будет вместо пути false
 *
 * @param assetsPath - папка в проекте, где лежат собранные обновления
 * @param assetMask - путь до файла обновления, не включая assetsPath
 * @param options - общие данные
 * @param exception - выдавать сообщение об ошибке или нет
 * @returns {*}
 */
...
/**
 * Возвращает список существующих ключей (builds) массива ASSETS_DATA
 *
 * @param options
 * @returns {any[]}
 */
function getAvailableBuilds(options) {
  const assetsPath = path.resolve(options.path); // преобразует последовательность путей или сегментов путей в абсолютный
...
/**
 * Собирает и возвращает массив, в котором каждый элемент это массив, с ключом и путём
 * [ [key, path], [key, path] ]
 *
 * @returns {Array}
 */
function getInstallersArrayMap() {
  return Object.keys(ASSETS_DATA)
    .reduce((sum, key) => { // собирает все значения key в sum, в числовом массиве применяется для получения суммы
